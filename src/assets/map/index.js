...
class Map {
  constructor() {
    ...
    this.scale = 10
    this.highlight = null
    this.currentBaseMap = "satellite2D"//目前存在 gaodemap satellite2D
    this.currentView = {
      MapView: null,
      sceneView: null,
      activeView: null,
  ...
  static config = {
    ...
    MapServerUrl: "http://113.54.15.11:6080/arcgis/rest/services//shuangliu/MapServer?f=jsapi",
	coordinate:{
		x:104.040,
		y:30.459
	},
  ...
  initMap(options) {
    ...
    this.currentView.activeView.map = this.baseMap[this.currentBaseMap].map;//在这里切换默认底图
  ...
  _2D_click_hight(event, Vue) {
    this.currentView.MapView.hitTest(event).then((response) => {
      ...
      if (response.results[0]) {//如果点击到了东西
        if (this.highlight) {
          this.highlight.remove()
        }
